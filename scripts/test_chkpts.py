...
for path in dir_path:
    if os.path.exists(os.path.join(path, 'accuracy.csv')):
        print("Already tested")
        continue
    writer = SummaryWriter(path, filename_suffix='_eval_chkpt')
    convmlp = ConvMLP(worker_idx=-1, num_client=1, num_shards=1, batch_size=128 * 5)
    ...
    with open(config, 'r') as f:
        print(f.readlines())
    if not os.path.exists(os.path.join(path, 'chkpt')):
        print("No chkpt")
        continue
    ...
    csv_writer.writerow(['time', 'iteration', f'accuracy/{path}'])
    with torch.no_grad():
